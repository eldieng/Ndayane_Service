...
interface LigneSortie { produitId: string; quantite: number }
...
export default function SortieStockPage() {
  ...
  const [lignes, setLignes] = useState<LigneSortie[]>([{ produitId: "", quantite: 1 }])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      for (const ligne of lignes) {
        const res = await fetch("http://localhost:3001/stock/sortie", {
          method: "POST",
      ...
      showToast("Sortie de stock enregistrée avec succès", "success")
      router.push("/stock")
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div><h1 className="text-2xl font-bold">Sortie de Stock</h1><p className="text-gray-500">Retirer des produits du stock</p></div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-sm border p-6 space-y-6">
        ...
        <div>
          <div className="flex justify-between items-center mb-4">
            ...
            <button type="button" onClick={addLigne} className="flex items-center gap-1 text-amber-500 hover:text-amber-700 text-sm"><Plus className="w-4 h-4" />Ajouter</button>
          </div>
        ...
        <div className="flex justify-end gap-4 pt-4 border-t">
          ...
          <button type="submit" disabled={saving} className="flex items-center gap-2 px-6 py-2 bg-red-600 hover:bg-red-700 text-white rounded-lg disabled:opacity-50">
